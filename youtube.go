...
func (svc *Service) SearchAndDownload(query string) (*Media, error) {
...
func (svc *Service) doSearchAndDownload(query string) searchAndDownloadResult {
	fmt.Println("searching and downloading")
	...
	if err != nil {
		...
		args := []string{
			...
			"--max-downloads", "1",
			"--output", fmt.Sprintf("%s/%d.opus", svc.fileDirectory, start.Unix()),
			"--quiet",
			"--print-json",
			"--ignore-errors",
			"--no-color",
